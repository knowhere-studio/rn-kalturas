...
- (void)viewDidLoad {
    ...
    [self registerReadyEvent:^{
        [self asyncEvaluate:@"{mediaProxy.entry.tags}" expressionID:@"Tags" handler:^(NSString *value) {
            if(value != nil && ![value isEqual:[NSNull null]]) {
                NSLog(@"%@", value);
                NSArray *tags = [value componentsSeparatedByString:@","];
                if([tags containsObject:@"360"]) {
                    [self.kPlayer changeTo360Player:self];
                }
            }
    ...
    switch ( attributeName.attributeEnumFromString ) {
        ...
        case playerError:
            if ([_delegate respondsToSelector:@selector(kPlayer:didFailWithError:)]) {
                NSDictionary *dict = @{NSLocalizedDescriptionKey:attributeVal, NSLocalizedFailureReasonErrorKey:attributeVal};
            ...
            break;
        case licenseUri:{
            NSString * entryID = _currentConfiguration.entryId;
            NSString * partnerID = _currentConfiguration.partnerId;
            NSString * izSessions = _currentConfiguration.izsession;
            
            NSString *licenseURL = [NSString stringWithFormat:@"%@?entry_id=%@&izsession=%@&partner_id=%@", attributeVal, entryID, izSessions, partnerID];
            _playerFactory.licenseUri = licenseURL;
            break;
        }
        case fpsCertificate:
