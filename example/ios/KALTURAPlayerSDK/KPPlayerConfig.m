...
- (void)loadUIConfWithCompletionHandler:(void (^)(NSDictionary * __nullable uiconf, NSError * __nullable error))handler {
    ...
    if (serverURL.absoluteString.length > 0) {
        ...
        KPLogError(@"serverURL is null");
        NSError *error = [NSError errorWithDomain:@"KPLocalAssetsManager"
                                    code:'uice'
                                userInfo:@{NSLocalizedDescriptionKey: @"UIConf service error",
                                           @"serverURL:": serverURL ? serverURL : @"<none>",
                                           @"Server String:": self.server ? self.server : @"<none>"}];
